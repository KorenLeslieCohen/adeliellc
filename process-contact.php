...
if (isset($_POST['action'])) { // Checking for submit action
	$my_email = 'aaron@adeliellc.com'; // Change with your email address
	
	if ($_POST['action'] == 'add') {
		$name	 = trim(strip_tags(addslashes($_POST['name'])));
		$email	 = trim(strip_tags(addslashes($_POST['email'])));
		$subject_test = trim(strip_tags(addslashes($_POST['subject'])));
		$subject = "Adélie, LLC Website Inquiry: ".$subject_test;
